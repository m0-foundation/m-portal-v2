...
abstract contract HyperlaneBridgeAdapterUnitTestBase is Test {
    ...
    HyperlaneBridge internal implementation;
    HyperlaneBridge internal adapter;
    ...
    function setUp() public virtual {
        ...
        implementation = new HyperlaneBridge(address(mailbox), address(portal));

        // Deploy UUPS proxy
        bytes memory initializeData = abi.encodeCall(HyperlaneBridge.initialize, (admin, operator));
        ERC1967Proxy proxy = new ERC1967Proxy(address(implementation), initializeData);
        adapter = HyperlaneBridge(address(proxy));
