...
/// @dev Regression tests for the $M earner rounding tolerance in `HubPortal._revertIfInsufficientMReceived`.
///
...
///      `index / 1e12 + 1` wei (1-2 wei at the current index). With a strict
///      `actualAmount < specifiedAmount` check, every such send reverts with
///      `InsufficientAmountReceived`; the rounding tolerance makes them succeed. This is independent
///      of the Wrapped $M version - it fires on the plain $M path where no extension is involved at all.
///
///      IMPORTANT: `HubPortalForkTestBase.setUp` does not upgrade the proxy to the locally compiled
///      implementation, so the regular fork tests exercise the currently deployed bytecode rather
///      than this branch's source. Each test below first re-applies the local build to the proxy so
///      the sweep runs against this branch's `Portal.sol` and `HubPortal.sol`.
///
///      The tests sweep consecutive amounts and assert every send succeeds. Without the rounding
///      tolerance most amounts in the sweep revert at the pinned block (after a failed send the
///      Portal's principal fraction is unchanged, so failures run in consecutive streaks).
///      Run with:
...
contract EarnerRoundingRevertForkTest is HubPortalForkTestBase {
    ...
    function _sweep(address sourceToken, bytes32 destinationToken) internal {
        ...
        for (uint256 amount = SWEEP_START; amount <= SWEEP_END; ++amount) {
            try hubPortal.sendToken{ value: fee }(amount, sourceToken, BNB_CHAIN_ID, destinationToken, recipient, refundAddress, "") { }
